...
public class WorldChannelModule extends AbstractModule<DiscordSRV> {
    ...
    public void onGameMessagePreProcess(AbstractGameMessagePreProcessEvent event) {
        String worldName = event.getPlayer().world();
        if (worldName == null || worldName.isEmpty()) {
            return;
        }

        GameChannel worldChannel = discordSRV.channelConfig().resolveChannel(worldName);
        ...
        if (messageConfig == null) {
            // No config available for this event type on that channel, skip routing
            return;
        }
    ...
    private IMessageConfig messageConfigForEvent(ServerBaseChannelConfig channelConfig, AbstractGameMessagePreProcessEvent event) {
        if (event instanceof GameChatMessagePreProcessEvent) {
            return channelConfig.minecraftToDiscord;
        }

        if (event instanceof JoinMessagePreProcessEvent) {
            return channelConfig.joinMessages().getForEvent((JoinMessagePreProcessEvent) event);
        }

        if (event instanceof LeaveMessagePreProcessEvent) {
            return channelConfig.leaveMessages;
        }

        if (event instanceof DeathMessagePreProcessEvent) {
            return channelConfig.deathMessages;
        }

        if (event instanceof AwardMessagePreProcessEvent) {
            return channelConfig.awardMessages;
        }
