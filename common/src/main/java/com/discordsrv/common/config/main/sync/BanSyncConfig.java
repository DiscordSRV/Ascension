...
public class BanSyncConfig extends AbstractSyncConfig<BanSyncConfig, Game, Long> {
    ...
    public static class DiscordToMinecraftConfig {
        ...
        @Comment("What action(s) on Discord should trigger a ban/unban in Minecraft. Valid options:\n"
                ...
                + "BEWARE: Settings of 'role' or 'either' can be exploited to remove bans from players if %1 is set to 'discord', and %2 or %3 aren't set to `true`")
        @Constants.Comment({"tie-breaker", "prefer-discord-on-first-join", "prefer-discord-on-account-link"})
